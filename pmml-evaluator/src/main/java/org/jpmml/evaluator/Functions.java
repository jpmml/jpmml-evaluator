...
public interface Functions {
		...
		public Number evaluate(Number base, Number exponent){
			...
			if(base instanceof Integer && exponent instanceof Integer){
				...
				if(exponent.intValue() < 0){
					throw new InvalidArgumentException(getName(), 1, InvalidArgumentException.formatMessage(getName(), "exponent", exponent) + ". Must be equal to or greater than 0");
				}
	...
	TernaryFunction SUBSTRING = new TernaryFunction(PMMLFunctions.SUBSTRING, Arrays.asList("input", "startPos", "length")){
		...
		public String evaluate(String string, int position, int length){
			...
			if(position < 1){
				throw new InvalidArgumentException(getName(), 1, InvalidArgumentException.formatMessage(getName(), "startPos", position) + ". Must be equal to or greater than 1");
			} // End if

			if(length < 0){
				throw new InvalidArgumentException(getName(), 2, InvalidArgumentException.formatMessage(getName(), "length", length));
			}
	...
	BinaryFunction FORMAT_NUMBER = new BinaryFunction(PMMLFunctions.FORMATNUMBER, Arrays.asList("input", "pattern")){
		...
		public String evaluate(Number input, String pattern){
			...
			try {
				...
				throw new InvalidArgumentException(getName(), 1, InvalidArgumentException.formatMessage(getName(), "pattern", pattern))
					.initCause(ife);
			}
		}
